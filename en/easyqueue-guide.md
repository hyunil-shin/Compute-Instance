...
* To view the detailed information of a topic, click the **View Details** button in the action column.

### Create Topic

1. To create a topic, click the **Create Topic** button.
2. Enter the topic information.
    * **Name**: The name of the topic. The Appkey is prefixed to the entered name when created.
    ...
    * **Maximum storage size per partition**: The maximum data size that can be stored in each partition. When this size is reached, the oldest messages are deleted first.
      Deletion is performed in segment file units (1GB), and since the segment currently being written to is not deleted, the actual size may exceed the configured value by approximately 1GB or more.
...
This is the screen where you can view the detailed information of a topic. You can check partition, message, consumer group, and monitoring information.

### Partition
* The list of partitions for the EasyQueue topic is displayed.
* You can view partition ID, start offset, last offset, and total message count information.

### Message
* The list of messages for the EasyQueue topic is displayed. A maximum of 50 messages are retrieved.
* You can check message send time, partition ID, offset, key, value, and header information.
* Click the **View Message** button in the action column to view detailed information about the key, value, and header.
...
1. To perform a message send test, click the **Message Send Test** button.
...
!!! tip "Note"
    When performing a message send test, you can send an empty message without entering any of the partition ID, header, key, or value.

### Consumer Group
* The list of consumer groups for the EasyQueue topic is displayed. A maximum of 50 consumer groups are retrieved.
* You can view consumer group ID, number of consumers, assigned partitions, total lag, and status information.
* Clicking an item displays the consumer list information tab at the bottom for the consumer group.
    * The list of consumers belonging to the consumer group is displayed.
    * You can view consumer ID, partition ID, lag, current offset, and last offset information.

!!! danger "Caution"
    * If a consumer group remains in a 'No members' state for more than 7 days, the group information and offsets are automatically deleted.


### Monitoring
* Through EasyQueue topic monitoring, you can check metrics for inbound/outbound byte transfer rates, message count, lag by consumer group, and total data size of the topic.

| Metric | Description |
|------|------|
| **Inbound / Outbound byte transfer rate** | **Inbound**: Data throughput sent by producers to the topic<br>**Outbound**: Data throughput read by consumers from the topic |
| **Message count** | Indicates the total number of messages stored in the topic. |
| **Consumer group lag** | The number of messages that the consumer group has not yet processed. It represents the difference between the latest message produced by the producer and the last message read by the consumer.<br>The consumer group lag monitoring metric displays only the top 10 results based on the highest consumer lag values at the current point in time. |
| **Data storage size** | The disk storage space currently being used by the topic.<br>The legend is displayed per partition of the topic, and both leader partitions and replica partitions are shown. |
